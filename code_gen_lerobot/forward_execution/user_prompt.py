...
5. **Subtask pattern**: Wrap each logical unit of work with `set_subtask()` before and `clear_subtask()` after.
   ...
   - **CRITICAL (color block sort order)**: When sorting multiple *colored blocks* (e.g., red/green/blue/yellow) onto matching-color plates or targets, ALWAYS process them in this fixed sequence: **red → green → blue → yellow → orange → purple → ...** (rainbow / wavelength order). Even if the instruction does not explicitly state the order, generate `set_subtask` calls in this canonical color sequence — never reorder by physical distance, ease of grasp, or scene layout. Reason: downstream Phase2 replay + judge expect a deterministic color order.
