...
# -------- policy 설정 --------
POLICY_PATH="${POLICY_PATH:-CoRL2026-CSI/smolVLA_Ours_Sim2RealO_OpenDrawer_100epi_50ep}"   # HF model ID 또는 로컬 체크포인트 경로
...
ROBOT_PORT="${ROBOT_PORT:-/dev/ttyACM2}"              # robot0 시리얼 포트
...
_cam_entry() {
    local type="$1" id="$2" w="$3" h="$4" fps="$5" fourcc="${6:-}" warmup_s="${7:-1}"
    if [ "$type" = "opencv" ]; then
        if [ -n "$fourcc" ]; then
            echo "{type: opencv, index_or_path: $id, width: $w, height: $h, fps: $fps, fourcc: '$fourcc', warmup_s: $warmup_s}"
        else
            echo "{type: opencv, index_or_path: $id, width: $w, height: $h, fps: $fps, warmup_s: $warmup_s}"
        fi
    else
...
if [ -z "${CAMERAS+x}" ]; then
    _lw=$(_cam_entry "$CAM_LEFT_WRIST_TYPE" "$CAM_LEFT_WRIST_ID" "$CAM_LEFT_WRIST_WIDTH" "$CAM_LEFT_WRIST_HEIGHT" "$CAM_LEFT_WRIST_FPS" "$CAM_LEFT_WRIST_FOURCC" "$CAM_LEFT_WRIST_WARMUP_S")
...
# -------- task / 실행 설정 --------
TASK="${TASK:-Open the top drawer.}"                     # language instruction
...
RTC_EXECUTION_HORIZON="${RTC_EXECUTION_HORIZON:-24}"  # chunk 당 실행 스텝 수
NON_RTC_CHUNK_STEPS="${NON_RTC_CHUNK_STEPS:-0}"       # RTC=false일 때 chunk 앞 N step만 실행 (0=전체)
...
========= VLA Inference =========
 ...
 cameras : $CAM_LEFT_WRIST_NAME($CAM_LEFT_WRIST_TYPE:$CAM_LEFT_WRIST_ID ${CAM_LEFT_WRIST_WIDTH}x${CAM_LEFT_WRIST_HEIGHT}@${CAM_LEFT_WRIST_FPS}fps fourcc=${CAM_LEFT_WRIST_FOURCC} warmup=${CAM_LEFT_WRIST_WARMUP_S}s)
...
if [ "$CAM_LEFT_WRIST_TYPE" = "opencv" ] && [ "${CAM_LEFT_WRIST_PREFLIGHT_S:-0}" != "0" ]; then
    CAM_LEFT_WRIST_ID="$CAM_LEFT_WRIST_ID" \
