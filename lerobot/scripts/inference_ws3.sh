...
# lerobot/src = lerobot core, $ADC_DIR/src = lerobot_cap (KinematicsEngine 등)
ADC_DIR="$(cd "$REPO_DIR/.." && pwd)"
export PYTHONPATH="$REPO_DIR/src:$ADC_DIR/src${PYTHONPATH:+:$PYTHONPATH}"

# -------- policy 설정 --------
POLICY_PATH="${POLICY_PATH:-CoRL2026-CSI/smolVLA_Ours_Sim2RealX_PickandPlace_100epi_50ep}"   # HF model ID 또는 로컬 체크포인트 경로
...
# -------- task / 실행 설정 --------                     # language instruction (run_forward_and_reset_ws3.sh INSTRUCTION 과 동일)
...
# -------- 카메라별 mp4 로깅 (inference 전체 구간) --------
# inference 진행 중 매 step 의 카메라 frame 을 cam 별 mp4 로 저장.
# Enter-park preview 와 독립 (PauseCameraWindow 와 별도 background thread).
# 출력: $LOG_CAMERAS_DIR/top.mp4, $LOG_CAMERAS_DIR/left_wrist.mp4 등.
LOG_CAMERAS_ENABLED="${LOG_CAMERAS_ENABLED:-true}"
LOG_CAMERAS_DIR="${LOG_CAMERAS_DIR:-$REPO_DIR/results/inference_cam_log_$(date +%Y%m%d_%H%M%S)}"
LOG_CAMERAS_FPS="${LOG_CAMERAS_FPS:-30}"
LOG_CAMERAS_NAMES="${LOG_CAMERAS_NAMES:-top,left_wrist}"

# -------- FK 기반 EE pitch clamp (ADC transit_pitch_max_deg 정확 동등) --------
# inference action 의 joints → FK → gripper pitch 계산 → 한계 초과 시 wrist_flex 조정.
# Convention: pitch=0=horizontal, -90°=down. -25° = "gripper 가 horizontal 보다 ≥25° 아래" 강제.
INFERENCE_PITCH_MAX_DEG="${INFERENCE_PITCH_MAX_DEG:--30}"
...
========= VLA Inference =========
 ...
 camlog  : enabled=$LOG_CAMERAS_ENABLED  cams=$LOG_CAMERAS_NAMES @ ${LOG_CAMERAS_FPS}fps  dir=$LOG_CAMERAS_DIR
 pitch   : max=${INFERENCE_PITCH_MAX_DEG:-none}°  (FK-based, gripper Z arcsin; 0=horizontal, -90=down)
...
INFER_ARGS=(
    ...
    --log_cameras_enabled="$LOG_CAMERAS_ENABLED"
    --log_cameras_dir="$LOG_CAMERAS_DIR"
    --log_cameras_fps="$LOG_CAMERAS_FPS"
    --log_cameras_names="$LOG_CAMERAS_NAMES"
)
...
if [ -n "$INFERENCE_PITCH_MAX_DEG" ]; then
    INFER_ARGS+=(--inference_pitch_max_deg="$INFERENCE_PITCH_MAX_DEG")
fi
