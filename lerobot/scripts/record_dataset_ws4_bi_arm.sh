#!/usr/bin/env bash
# =============================================================================
#  SO-101 Dual-Arm Dataset Recording — WS4 (H.264 codec, bi_so_follower)
#
#  ws1 dual-arm 스크립트(record_dataset_ws1_bi_arm.sh)와 동일 구조.
#  WS4 전용 차이점:
#    - follower:  left=so101_robot6 (/dev/ttyACM1)
#                 right=so101_robot7 (/dev/ttyACM4)
#    - leader:    so101_robot2_leader (left,  /dev/ttyACM3)
#                 so101_robot3_leader (right, /dev/ttyACM5)
#    - top cam:   RealSense serial 254622075836 (WS4 전용)
#    - wrist cam: /dev/video2 (left), /dev/video16 (right)  ※ 본 머신 v4l 매핑 기준
#
#  Usage:
#    ./record_dataset_ws4_bi_arm.sh
#    REPO_ID=my_user/dual_dataset NUM_EPISODES=20 ./record_dataset_ws4_bi_arm.sh
#    VCODEC=h264_nvenc ./record_dataset_ws4_bi_arm.sh
#    LEFT_FOLLOWER_PORT=/dev/ttyACM1 RIGHT_FOLLOWER_PORT=/dev/ttyACM4 \
#    LEFT_LEADER_PORT=/dev/ttyACM3   RIGHT_LEADER_PORT=/dev/ttyACM5 \
#      ./record_dataset_ws4_bi_arm.sh
#    TOP_ARM_SIDE=right ./record_dataset_ws4_bi_arm.sh
#    TOP_ARM_SIDE=none  ./record_dataset_ws4_bi_arm.sh
#    RESUME=true ./record_dataset_ws4_bi_arm.sh
#
#  데이터셋 키 (bi_so_follower 자동 prefix):
#    observation.images.left_wrist
#    observation.images.left_top    (TOP_ARM_SIDE=left default)
...
# -------- resume --------
RESUME="${RESUME:-false}"  # true → 기존 데이터셋에 이어서 기록 (episode 번호 자동 증가)

# -------- bi_so_follower (좌/우 follower) --------
ROBOT_TYPE="${ROBOT_TYPE:-bi_so_follower}"
ROBOT_ID="${ROBOT_ID:-so101_dual_robot6_robot7}"

LEFT_FOLLOWER_PORT="${LEFT_FOLLOWER_PORT:-/dev/ttyACM1}"
LEFT_FOLLOWER_ID="${LEFT_FOLLOWER_ID:-so101_robot6}"

RIGHT_FOLLOWER_PORT="${RIGHT_FOLLOWER_PORT:-/dev/ttyACM4}"
RIGHT_FOLLOWER_ID="${RIGHT_FOLLOWER_ID:-so101_robot7}"

# -------- bi_so_leader (좌/우 leader) — ws1과 동일 --------
...
RIGHT_LEADER_PORT="${RIGHT_LEADER_PORT:-/dev/ttyACM5}"
...
if [ -z "$LEFT_LEADER_PORT" ] || [ -z "$RIGHT_LEADER_PORT" ]; then
    ...
    echo "        예: LEFT_LEADER_PORT=/dev/ttyACM3 RIGHT_LEADER_PORT=/dev/ttyACM5 $0" >&2
...
# -------- cameras --------
# WS4 카메라 매핑:
#   left wrist:  OpenCV /dev/video2   640x480@30fps  (MJPG)   ※ Innomaker U20CAM
#   right wrist: OpenCV /dev/video16  640x480@30fps  (MJPG)   ※ Innomaker U20CAM
#   top (shared, RealSense):  254622075836  640x480@30fps      ※ WS4 전용
#
...
LEFT_WRIST_CAM_ID="${LEFT_WRIST_CAM_ID:-/dev/video2}"
...
TOP_CAM_ID="${TOP_CAM_ID:-254622075836}"
...
# -------- summary --------
cat <<EOF
======= SO-101 Dual-Arm Dataset Recording — WS4 (H.264, bi_so_follower) =======
 ...
 leader    : $TELEOP_TYPE ($TELEOP_ID)   ※ ws1과 동일
