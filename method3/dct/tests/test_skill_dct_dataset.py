...
class _FakeBaseDataset:
    ...
    def __init__(self):
        # frame_start = 0, 30 → 두 skill segment 사용.
        self._frames = {
            0: {
            ...
            30: {
                "observation.images.top": torch.ones(3, 224, 224),
...
def test_observation_columns_preserved(sidecar_parquet):
    ...
    # frame_start=0 vs 30 의 base 값 차이가 보존돼야 함.
    assert torch.equal(item0["observation.state"], torch.zeros(6))
    assert torch.equal(item1["observation.state"], torch.ones(6))
