...
### ==================== [single arm task] ==================
## drawer open (5개 phase2 추가 → session_20260523_220703 resume)
INSTRUCTION="Open the top drawer 7cm by pulling drawer handle."
...
export_realsense_serial_from_recording_config() {
    local config_file="$RECORDING_CONFIG_FILE"
    if [ ! -f "$config_file" ]; then
        return
    fi
    local rs_serial
    rs_serial="$(python3 - "$config_file" <<'PY'
import sys
import yaml
...
for cam in cfg.get("cameras", {}).get("shared", []):
    if (
        cam.get("enabled", True)
        and str(cam.get("type", "")).lower() == "realsense"
        and cam.get("serial_number")
    ):
...
PY
    ...
    if [ -n "$rs_serial" ]; then
        export VOXPOSER_RS_SERIAL="$rs_serial"
        echo "[Config] VOXPOSER_RS_SERIAL=$VOXPOSER_RS_SERIAL ($(basename "$config_file") top camera)"
    fi
}
...
# Recording config 로드
# - RECORD_DATASET=true: dataset repo/fps 설정까지 사용
# - RECORD_DATASET=false: dataset 저장은 안 하지만, direct camera가 같은 top
#   RealSense를 쓰도록 serial만 export
load_recording_config
export_realsense_serial_from_recording_config
