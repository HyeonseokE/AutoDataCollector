...
green "  (3) run_forward_and_reset_ws*.sh 2곳 수정 후 실행:"
green "        PHASE=\"phase1\"  →  PHASE=\"phase2\""
green "        RESUME_SESSION=\"\"  →  RESUME_SESSION=\"$SESSION_DIR\""
green "      → 기존 session 이어받기 (resume 모드). Phase2 episode 는"
green "        session/phase2/ 하위에 phase1 episode 번호 이어서 저장,"
green "        phase1 의 seed_*_setup 그대로 재사용 (seed 추가 생성 X)."
